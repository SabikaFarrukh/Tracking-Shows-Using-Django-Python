...
def showlist(request, username: str = ""):
    ...
    if sort_type == 2:
        results = UserR.objects.filter(user=user).order_by('pk')
    elif sort_type == 3:
        results = UserR.objects.filter(user=user).order_by('-rating')
    elif sort_type == 4:
        results = UserR.objects.filter(user=user).order_by('rating')
    else:
        results = UserR.objects.filter(user=user).order_by('-pk')
...
def show(request, showid: int = 0):
    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            if "rating" in request.POST:
                ...
                if value == 0:
                    UserR.objects.get(show=sh, user=request.user).delete()
                    return render(request, 'tv/show.html', {'show': resp, 'title': resp['name'], 
                                                            'watched': False, 'rating': 0})
                else:
                    try:
                        ur = UserR.objects.get(show=sh, user=request.user)
                    except UserR.DoesNotExist:
                        ur = UserR(show=sh, user=request.user)
                    ur.rating = value
                    ur.save()
            ...
            if "#" in request.POST:
                try:
                    ...
                    value = 0
                try:
                    ur = UserR.objects.get(user=request.user, position=value)
                    ur.position = 0
                    ur.save()
                except UserR.DoesNotExist:
                    pass
    ...
    try:
        ur = UserR.objects.get(show=Show .objects.get(showid=showid), user=request.user)
        return render(request, 'tv/show.html', {'show': resp, 'title': resp['name'],  'watched': True,
                                                'rating': ur.rating,
                                                'position': ur.position})
    except UserR.DoesNotExist:
